...
router.get('/', function(req, res, next) { 
  WebStatisticsPage.find({}, function(error, doc) {
    // TODO These erors don't make sense
    if(error) return next(produceError('serverMaintenance', 503))
    if(!doc) return next(produceError('serverMaintenance', 503))
    return res.json(doc)
  })
